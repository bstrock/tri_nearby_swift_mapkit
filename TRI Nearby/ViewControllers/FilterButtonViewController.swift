...
class FilterButtonViewController: UIViewController {
    ...
    @IBAction func applyFilters(_ sender: Any) {
        ...
        if let vc = presentingViewController as? ViewController {
            ...
            fetchSitesJsonData(query: query) { (incomingSites) in
                ...
                // record # of returned results on top-level button for user interpretation
    ...
    @IBAction func clearFilters(_ sender: Any) {
        // used to reset filter values and map view to default
        ...
        // reset the persistent state object which represents the user's filter selections
        if let vc = presentingViewController as? ViewController {
        ...
        applyFilters(sender)  // performs the view and filter reset in the UI
        dismiss(animated: true, completion: nil)  // bye bye filter view
    }
...
// TODO:  ASSESS AND REFACTOR THESE
