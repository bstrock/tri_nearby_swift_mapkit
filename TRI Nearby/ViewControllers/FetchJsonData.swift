...
func fetchSitesJsonData(
    ...
    URLSession.shared.dataTask(with: url) { data, response, error in
        guard let data = data else {
            return
        } // checks data integrity; null responses from api fail here
...
func postReportJsonData(
    ...
    // this function used to send json representing user report to API
    // configure request
    var component = URLComponents()
    component.scheme = "https"
    component.host = "tri-nearby-db.herokuapp.com"
    ...
    do {
        // cannot serialize using JSONEncoder().encode(report)- muse use JSONSerialization + report.jsonDictionary()
    ...
    // send request
    URLSession.shared.dataTask(with: request) { data, response, error in  // this is an async function!
